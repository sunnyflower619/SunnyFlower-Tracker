...
2. Create a new Google Sheet using [this template](https://docs.google.com/spreadsheets/d/1feZzdIMOgfjJjh4daZTj0wuJHf6bZkDKXLZ1PIr8vdY/edit?usp=sharing). You will note there are some _default values_ for the Tags sheet. This is normal. If you are on ANY sites where you have immortals and would like to utilize the "immortal vs mortal" toggle on the character sheets, you _must_ at minimum keep "true age". You _can_ (and, for the age filters, _should_) change the sites they are assigned to in the Edit Tags form; by default I have set them to "all" as this is the only site-agnostic option. The difference between the age filters is as follows:
   1. **Age:** For sites with no immortals. Has normal age ranges.
   2. **True Age:** For sites with immortals. Has expanded age ranges and will add mortal or immortal tags based on how you toggle this field in the Add Character form.
   3. **Visual Age:** For sites with immortals. Just an extra filter to help you out if their real age does _not_ match their visual age.
